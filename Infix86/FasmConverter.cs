...
namespace Infix86
{
    ...
    public class FasmConverter
    {
        ...
        private List<string> _invalidTokens;

        public List<string> InvalidTokens => _invalidTokens;

        public bool IsValid => !_invalidTokens.Any();
        ...
        public FasmConverter(string postfix)
        {
            _postfix = postfix;
        ...
        public string ToAsm()
        {
            ...
            var asm = new StringBuilder(";FASM" + Constants.NewLine);
            ...
            {
                asm.Append(";" + variable.Key + Constants.NewLine);
        ...
        private void IdentifyVars()
        {
            ...
            {
                foreach (var op in line.Split(new[] { Constants.Space }, StringSplitOptions.RemoveEmptyEntries)) // & x y
                {
        ...
        private Dictionary<string, string> GetAssignedVars()
        {
            ...
            foreach (var line in exprLines)
            {
                var ops = line.Split(new[] { Constants.Space }, StringSplitOptions.RemoveEmptyEntries); // & x y
